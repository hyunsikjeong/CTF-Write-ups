...
class EMU:
    ...
    def assembly(self):
        op_name = [ 'x' + str(i) for i in range(31) ]
        ...
        reg_name = ['r0', 'r1', 'r2', 'r3', 'r4', 'r5', 'r6', 'r7','r8', 'r9', 'r10', 'bp', 'sp', 'pc', 'eflags', 'zero']
        
        try:
            while 1:
                cur_pc = self.register.get_register('pc')
                op, op_type, opers, op_size = self.dispatch_assem(cur_pc)
                ...
                s += " %5s" % op_name[op]
                if op_type == TYPE_R:
                    s += " R %s %s" % (reg_name[opers[0]], reg_name[opers[1]])
                else:
                    s += " I %s I%d" % (reg_name[opers[0]], opers[1])

                if op >= 25 and op <= 30:
                    tmp = cur_pc + op_size + opers[1]
                    if tmp > 2 ** 21:
                        tmp -= 2 ** 21
                    s += " # %s %d" % (op_name[op], tmp)

                if op == 7 and opers[0] == 13:
                    s += '\n'
                    
                print(s)
                #print(op, op_type, opers, op_size)
                
        except:
            self.terminate("[VM] Unknown error")

    def dispatch_assem(self, addr):
